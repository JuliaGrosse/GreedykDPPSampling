...
plt.rcParams.update(bundles.aistats2023(column="full", nrows=1, ncols=2))
...
algorithms = ["greedy","mcmc_discrete","exact", "exact_vfx"]
discretizations = [10, 15, 20, 25, 30, 35, 40, 50, 60, 70, 80, 90]
ks = [10, 100]
ells = [0.01, 0.001]
reps = range(1, 6)
...
def get_filename(algorithm, discretization, k, ell, rep, nb_samples=100):
    filename = (
        "./runtime_comparison_higher_dim/results/"
        + algorithm
        + "_discretization"
        + str(discretization)
        + "_nbsamples"
